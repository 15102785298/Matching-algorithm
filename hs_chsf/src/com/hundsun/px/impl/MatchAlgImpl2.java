...
public class MatchAlgImpl2 implements chsfInterface {

	private static Map<String, TradeList> chdl = new ConcurrentHashMap<String, TradeList>();
	...
	private List<RepBean> CalculatSel(TradeList czdl, ReqBean selReq) {
		...
		if (isDeal) { // 如果是有多次交易这种情况，此时该任务一定是插入到卖出队列头
			...
			//
			// while (low < high) {
			// if (selList.get(middle).getWtjg() < selJg) {
			// low = middle + 1;
			// } else if (selList.get(middle).getWtjg() > selJg) {
			// high = middle - 1;
			// } else {
			// high = middle;
			// }
	...
	public List<RepBean> MatchCalculationMul(List<ReqBean> mulReqBean) {
		...
		for (Iterator<ReqBean> iterator = mulReqBean.iterator(); iterator.hasNext();) {
			...
			//System.out.println("返回用时" + (endTime - startTime));
		}
		return res;
	}
}
